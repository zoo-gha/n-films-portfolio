...
export default function Navbar() {
    ...
    return (
        <nav className={`fixed w-full left-0 top-0 z-[999] ${sticky ? 'md:bg-white text-black shadow-xl' : 'text-white' }`}>
            <div className="flex items-center justify-between">
                <div className="mx-7">
                    <Link href={"/"}>
                        <img className="w-25 h-12 cursor-pointer rounded-lg " src={`${sticky ? '/n-films.png' : '/logo.jpg'}`} alt="" />
                    </Link>
                </div>
                <div className={`{${sticky ? 'md:bg-white/0 text-black' : 'text-white'}   lg:block hidden px-7 py-4 font-medium bg-none`}>
