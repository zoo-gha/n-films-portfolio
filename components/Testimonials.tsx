...
export default function Testimonials() {
  ...
  const settings = {
    ...
    afterChange: (current: any) => {
      setProgress(
        (100 / (testimonials_data.length - slideToShow + 1)) * (current + 1)
      );
    },
  };
  return (
    <section className="font-mont text-black -z-1">
      <div className="text-center py-8">
        ...
        <h1 className="text-xl w-96 mx-auto leading-normal font-bold mb-12">
          ...
          <Slider {...settings}>
            {testimonials_data.map((val: any) => {
              return (
                <div
                  key={val.id}
                  className="bg-emerald-900 mx-auto duration-500 group-hover:opacity-60 hover:!opacity-100 group-hover:scale-[0.85] hover:!scale-100 cursor-pointer p-8 rounded-xl mix-blend-luminosity"
                >
                  <img
                    src={val.img}
                    alt=""
                  ...
                  <button className="bg-emerald-200 text-black/50 py-2.5 px-8 rounded-full hover:bg-emerald-300 hover:text-white">
                    {" "}
                    Get in Touch
                  </button>
                </div>
